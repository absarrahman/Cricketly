    ...
    private func setDataBasedOnResult(_ result: Result<([FixtureModel]?), Error>, _ type: FixtureType) {
        ...
        case .success(let data):
            ...
            self.fixtureDataList = fixtureList.compactMap({ model in
                if (type != .recent) {
                    return FixtureCellModel(isLive: false, localTeamCode: model.localteam?.code ?? "N/A", visitorTeamCode: model.visitorteam?.code ?? "N/A", localTeamRun: "", visitorTeamRun: "", localTeamWicket: "", visitorTeamWicket: "", localTeamOver: "", visitorTeamOver: "", matchNote: model.startingAt ?? "N/A", matchType: model.type ?? "N/A", localTeamImageUrl: model.localteam?.imagePath ?? "N/A", visitorTeamImageUrl: model.visitorteam?.imagePath ?? "N/A")
                }
